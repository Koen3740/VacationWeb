...
function countFieldPresence(offers: StoredOffer[], field: keyof StoredOffer): number {
  return offers.filter((offer) => hasValue(offer[field])).length;
}

function sampleMissingOffers(
  offers: StoredOffer[],
  field: keyof StoredOffer,
  limit: number,
): StoredOffer[] {
...
function main() {
  ...
  const prijsvrijStored = importPrijsvrijXml(prijsvrijXml);
  const corendonStored = importCorendonXml(corendonXml);

  const prijsvrijNormalized = prijsvrijStored.map(normalizeOffer);
  const corendonNormalized = corendonStored.map(normalizeOffer);

  const prijsvrijFieldCoverage = Object.fromEntries(
    STORED_OFFER_FIELDS.map((field) => [
      field,
      {
  ...
  const corendonFieldCoverage = Object.fromEntries(
    STORED_OFFER_FIELDS.map((field) => [
      field,
      {
  ...
  const prijsvrijOnlyXmlProperties = PRIJSVRIJ_XML_PROPERTY_FIELDS.filter(
    (field) => !CORENDON_XML_PROPERTY_FIELDS.includes(field as (typeof CORENDON_XML_PROPERTY_FIELDS)[number]),
  );

  const corendonOnlyXmlProperties = CORENDON_XML_PROPERTY_FIELDS.filter(
    (field) => !PRIJSVRIJ_XML_PROPERTY_FIELDS.includes(field as (typeof PRIJSVRIJ_XML_PROPERTY_FIELDS)[number]),
  );

  const sharedXmlProperties = PRIJSVRIJ_XML_PROPERTY_FIELDS.filter((field) =>
    CORENDON_XML_PROPERTY_FIELDS.includes(field as (typeof CORENDON_XML_PROPERTY_FIELDS)[number]),
  );
  ...
  const report = {
    ...
    summary: {
      prijsvrijSourceFile: FEED_PATHS.prijsvrij,
      corendonSourceFile: FEED_PATHS.corendon,
    ...
    mapping: {
      ...
      prijsvrijFieldsAlwaysMissing: STORED_OFFER_FIELDS.filter(
        (field) => countFieldPresence(prijsvrijStored, field) === 0,
      ),
      corendonFieldsAlwaysMissing: STORED_OFFER_FIELDS.filter(
        (field) => countFieldPresence(corendonStored, field) === 0,
      ),
    },
    ...
    samples: {
      ...
      missingDepartureAirportExamples: sampleMissingOffers(prijsvrijStored, 'departureAirport', 3).map(
        (offer) => offer.externalId,
      ),
    },
  };

  const reportPath = path.join(process.cwd(), 'data', 'prijsvrij-validation-report.json');
  ...
  for (const field of ['price', 'country', 'region', 'city', 'boardType', 'departureDate', 'images', 'feedDescription', 'departureAirport', 'rating', 'province', 'descriptionLong'] as const) {
