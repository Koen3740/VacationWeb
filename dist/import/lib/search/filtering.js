...
const match_destination_1 = require("../destinations/match-destination");
function computeValueScore(offer) {
...
function filterOffers(offers, params) {
    return offers.filter((offer) => {
        if (params.country || params.region) {
            if (!params.country) {
                return false;
            }
            if (!(0, match_destination_1.offerMatchesDestination)(offer, {
        ...
        if (params.nightsMin !== undefined &&
            offer.nights < params.nightsMin) {
            return false;
        }
        if (params.nightsMax !== undefined &&
...
function sortOffers(offers, sort = 'value') {
    const ranked = offers.map((offer) => ({
        ...offer,
        valueScore: offer.valueScore ?? computeValueScore(offer),
        flexibilityScore: offer.flexibilityScore ??
            Math.max(70, 100 - Math.abs(offer.nights - 8) * 6),
    }));
    switch (sort) {
        case 'price':
            return [...ranked].sort((a, b) => a.price - b.price);
        case 'price-per-day':
            return [...ranked].sort((a, b) => a.pricePerDay - b.pricePerDay);
        case 'rating':
            return [...ranked].sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0));
        case 'stars':
            return [...ranked].sort((a, b) => (b.stars ?? 0) - (a.stars ?? 0));
        case 'value':
        default:
            return [...ranked].sort((a, b) => (b.valueScore ?? 0) - (a.valueScore ?? 0));
    }
}
